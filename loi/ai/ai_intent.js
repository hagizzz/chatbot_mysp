...
const KINDS = [
  ...
  "RISK_RECOURSE", // "mua về k giống được thế thì sao" / "hàng về không ưng thì sao" — hỏi QUYỀN LỢI khi hàng không như kỳ vọng. Khác AUTHENTICITY_QA (hỏi ảnh có thật không, TRƯỚC khi mua) và khác RETURN_POLICY (hỏi thẳng quy định đổi/trả). Đây là "THÌ SAO" = tôi được gì nếu chuyện đó xảy ra.
...
- "mua về mà k giống được thế thì sao shop" / "mua về không giống ảnh thì sao" / "hàng về không đẹp như hình thì sao" / "lỡ nhận hàng không ưng thì sao" / "nhỡ vải xấu hơn hình thì thế nào" / "không như quảng cáo thì sao" (khách hỏi HẬU QUẢ / QUYỀN LỢI CỦA MÌNH nếu hàng nhận về không như kỳ vọng — dấu hiệu nhận biết là cụm "THÌ SAO / THÌ THẾ NÀO / THÌ TÍNH SAO" đi cùng một giả định xấu) -> RISK_RECOURSE. Ba nhãn hay bị lẫn, phân biệt bằng CÂU HỎI THẬT của khách:
  • "ảnh có phải shop tự chụp không", "giống hình không" -> AUTHENTICITY_QA (hỏi về ẢNH).
  • "không vừa có đổi được không", "chính sách đổi thế nào" -> RETURN_POLICY (hỏi về QUY ĐỊNH, khách đã tự nghĩ tới chuyện đổi).
  • "mua về không giống thì SAO" -> RISK_RECOURSE (khách CHƯA biết mình có quyền gì, đang hỏi shop bảo đảm điều gì).
  TUYỆT ĐỐI KHÔNG gắn nhóm câu này thành SIZE_ADVICE: "không giống ảnh" là lo HÌNH THỨC/CHẤT LƯỢNG, không phải lo VỪA NGƯỜI. Chỉ khi khách nói rõ về size/dáng ("sợ chật", "sợ không vừa em") mới là SIZE_ADVICE.
...
- ĐỘNG TỪ quyết định, KHÔNG phải chữ "nữa"/"thêm": "tư vấn e mẫu này nữa" / "xem thêm mẫu này" / "cho hỏi mẫu này nữa" / "báo giá mẫu này nữa" / "còn mẫu này thì sao" (khách hỏi/xin TƯ VẤN về MỘT MẪU KHÁC, thường kèm ảnh mới) -> đây là HỎI SẢN PHẨM, để nhãn theo đúng câu hỏi (PRICE_ASK nếu hỏi giá, hoặc nhãn sản phẩm tương ứng), wants_order=FALSE. TUYỆT ĐỐI KHÔNG phải ORDER_CLOSE/ADD_TO_ORDER.
  Đo được trên page thật 24/08/2026: "tư vấn e mẫu này nữa" + ảnh bị đọc thành ORDER_CLOSE -> bot nhảy sang "em nhận được thông tin của chị rồi ạ, cho em xin số điện thoại để lên đơn" trong khi khách mới chỉ xin tư vấn. Khách chưa hề đồng ý mua.
  CHỈ khi động từ là MUA/LẤY/ĐẶT/CHỐT mới được tính ý mua: "lấy thêm cái này nữa" -> ADD_TO_ORDER; "lấy mẫu này" -> ORDER_CLOSE.
  MỞ RỘNG (đo 25/08/2026 — luật trên còn neo quá chặt vào chữ "nữa"): động từ TƯ VẤN / XEM GIÚP / THAM KHẢO / XEM HỘ ở ĐẦU câu là NHỜ TƯ VẤN, wants_order=FALSE, dù câu KHÔNG có chữ "nữa"/"thêm" và dù hội thoại đã từng chốt đơn. Ví dụ đã gắn nhãn SAI ngoài thật: "tư vấn cho mình những mẫu này với" + 3 ảnh -> bị đọc thành ORDER_CLOSE -> bot đi xin SỐ NHÀ trong khi khách mới chỉ nhờ xem mẫu.
  Khách gửi ẢNH MỚI mà chưa từng được báo giá mẫu đó => KHÔNG BAO GIỜ là ORDER_CLOSE, kể cả khi đã có đơn cũ trong hội thoại. Nhịp đúng là báo giá mẫu mới trước.
...
ĐỘ TIN CẬY (do_tin_cay) — BẮT BUỘC điền, số từ 0 đến 1:
Chấm THẬT mức chắc chắn của bạn về nhãn kind vừa chọn. KHÔNG được mặc định 1.
- 0.9-1.0 : câu rõ, khớp thẳng một ví dụ ở trên ("giá bao nhiêu", "0901234567", "cho xin bảng size").
- 0.7-0.85: hiểu được nhưng phải dựa vào "Tin shop ngay trước" / "Hội thoại gần đây" mới ra nhãn.
- 0.4-0.65: câu CỤT, viết tắt lạ, sai chính tả nặng, HOẶC có 2 nhãn đều hợp lý mà buộc phải chọn 1.
- < 0.4   : không thực sự hiểu khách muốn gì, chọn nhãn chỉ vì bắt buộc phải chọn. kind="OTHER" luôn <= 0.4.
Thà chấm THẤP còn hơn chấm cao rồi sai: điểm dưới ngưỡng thì hệ thống GIAO NGƯỜI THẬT xử,
không ai trả lời sai khách. Câu càng nhiều cách hiểu -> điểm càng thấp.

Trả JSON đúng khuôn:
{"kind":"<MỘT trong: ${KINDS.join("|")}>","size":"S|M|L|XL|XXL|FREESIZE|null","do_luong":"<SỐ ĐO khách đã cho trong TOÀN HỘI THOẠI (kể cả tin trước), dạng '1m55/42kg' hoặc '42kg' hoặc null nếu CHƯA TỪNG cho>","is_address":true/false,"has_phone":true/false,"phone":"<10 số đã tách sạch, hoặc null>","wants_order":true/false,"asks_price":true/false,"asks_size_chart":true/false,"address_complete":true/false,"province_confirm":true/false,"refers_to":"<tên/màu mẫu khách đang nhắc, hoặc null>","concern":"ngan|mong|chat|cogian|lot|dem|null","do_tin_cay":<số 0..1, vd 0.85>}`;
...
function _safe(obj) {
  const out = {
    ...
    refers_to: null,
    concern: null,
    do_tin_cay: null   // AI tự chấm 0..1. null = AI không chấm -> worker KHÔNG chặn (giữ hành vi cũ).
  };
  try {
    if (obj && typeof obj === "object") {
      ...
      // do_tin_cay: kẹp về [0,1]. Không phải số (AI quên điền / trả null / trả chữ) -> null,
      // KHÔNG tự bịa 1 (tin bừa) và KHÔNG để Number(null)=0 (hoá thành "không chắc" -> giao người mọi lượt).
      { const _raw = obj.do_tin_cay;
        const dtc = (typeof _raw === "number" || (typeof _raw === "string" && _raw.trim() !== "")) ? Number(_raw) : NaN;
        out.do_tin_cay = Number.isFinite(dtc) ? Math.max(0, Math.min(1, dtc)) : null; }
    }
  } catch (_) {}
  return out;
}
...
module.exports = { classifyIntent, KINDS, _safe };   // _safe xuất ra để test/do_tin_cay.test.js soi được luật kẹp điểm
