...
- LUẬT SỐ ĐO (BẮT BUỘC): khách ĐÃ nêu CHIỀU CAO và/hoặc CÂN NẶNG ở BẤT KỲ tin nào trong hội thoại => TUYỆT ĐỐI KHÔNG chọn HOI_SIZE. Chọn hanh_dong=TU_VAN, tin_nhan="" để hệ thống tư vấn size từ chính số đo đó. Hỏi lại thứ khách vừa đưa là lỗi nặng nhất trong mắt khách.
  Nhận CẢ dạng viết tắt và gõ nhanh: "1m6", "1m58", "m6", "m58", "1.60", "1,58", "cao 160", "160cm", "53kg", "53 kg", "nặng 53", "45kg 1m55", "cao mét sáu".
  PHÂN BIỆT: "HOI_SIZE" nghĩa là ĐI XIN SỐ ĐO CỦA KHÁCH — KHÔNG phải "lượt này có nói về size". Khách HỎI "mặc size gì / size nào vừa / size M được không" mà đã kèm số đo => TU_VAN, không phải HOI_SIZE.
  Chỉ dùng HOI_SIZE khi hội thoại CHƯA HỀ có số đo lẫn size khách tự khai.
...
- HOI_SIZE: "Dạ chị cho em xin chiều cao cân nặng để em tư vấn size chuẩn cho mình nha" (CHỈ khi hội thoại chưa có số đo nào — xem LUẬT SỐ ĐO ở trên)
...
- Địa chỉ: <địa chỉ đầy đủ>"
  TUYỆT ĐỐI không tự ghi số tiền, chỗ tiền ghi đúng {COD}. Khách nói "lấy 2 mẫu/cả 2/2 set" (kể cả ở tin cũ đã được nhân viên trả lời) -> đơn PHẢI đủ số mẫu đó, chọn đúng các mẫu khách đã xem/được báo giá. Kèm don.san_pham = danh sách [{ma,mau_sac,size,dia_chi}] + sdt + dia_chi.

## MỖI MÓN MỘT NƠI GIAO
Khách có thể yêu cầu giao mỗi món về một địa chỉ khác nhau ("1c trắng ship về địa chỉ cũ, 1c đen ship về 67 Nguyễn Xiển..."). Khi đó:
- Ghi địa chỉ của TỪNG món vào san_pham[i].dia_chi. KHÔNG nhét chung một địa chỉ cho cả đơn — làm thế là giao sai nhà.
- Món nào khách nói "địa chỉ cũ / như mọi khi / chỗ cũ / gửi về đấy" thì ghi ĐÚNG chuỗi "CU" (hai chữ cái), KHÔNG tự viết lại địa chỉ — hệ thống sẽ thay bằng địa chỉ đã lưu. Bịa lại địa chỉ cũ là giao sai nhà.
- Món nào khách không nói riêng thì để san_pham[i].dia_chi rỗng, hệ thống dùng don.dia_chi chung.
- don.dia_chi vẫn ghi địa chỉ MỚI khách vừa cho (nếu có), để dùng cho các món không nói riêng.

## MÀU PHẢI LÀ MÀU CÓ THẬT CỦA MẪU
mau_sac chỉ được lấy từ danh sách màu của mẫu trong danh mục. Khách nói một màu KHÔNG có trong danh sách đó (vd xin "trắng" mà mẫu chỉ có kem/hồng/đen) thì TUYỆT ĐỐI không tự đổi sang màu gần giống — để mau_sac rỗng, hệ thống sẽ hỏi lại khách. Tự đổi màu là giao sai hàng.

## ĐẦU RA (JSON duy nhất)
{"referent":"MÃ|UNKNOWN","do_tin_cay":0.0,"mau_cu":[],"dia_chi":{"trang_thai":"DU|THIEU|XAC_NHAN_TINH|KHONG_CO","thieu":[],"dia_chi_chuan":""},"hanh_dong":"...","tin_nhan":"...","don":{"san_pham":[{"ma":"","mau_sac":"","size":"","dia_chi":""}],"sdt":"","dia_chi":""}}`;
...
function _sanitize(obj) {
  ...
  items = items.map(it => ({
    ...
    // Địa chỉ RIÊNG của món này. "CU" = giao về địa chỉ đã lưu (đừng để AI viết lại
    // địa chỉ cũ — nó bịa là giao sai nhà). Rỗng = dùng don.dia_chi chung.
    dia_chi: _s(it && it.dia_chi, 250)
...
async function quyetDinh({ turns = "", candidatesText = "", known = "", timeoutMs = 7000 } = {}) {
  ...
  try {
    ...
    // KHAI NGUỒN: câu do tầng AI-QUYẾT soạn (không phải nhánh cứng, cũng không phải
    // kịch bản Doc) -> để bộ đếm tách bạch được ba đường sinh câu.
    try { turnLog.nguonCau("ai_quyet", out.tin_nhan, "ai_quyet.js"); } catch (_) {}
    return out;
  } catch (e) {
