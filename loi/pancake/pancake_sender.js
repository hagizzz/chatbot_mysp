...
const turnLog = require("../tien_ich/turn_log");   // ghi nhận bot đã gửi gì / gắn gỡ thẻ nào (mục 9.4)
...
// [FIX Hà Giang 2026-08-26] NHẬN GỢI Ý PAGE.
// Conv BÌNH LUẬN có id "POSTID_kháchId" -> pageIdFromConv trả về id BÀI VIẾT, không
// phải page. Nhiều page thì hàm rơi xuống PAGE_ID (.env = Mys.P) -> gửi comment id
// của PHOM sang page Mys.P -> Pancake trả "conversation_id not found" (error 120).
// Đo trên page thật: private_replies HỎNG 100% ở cấu hình đa-page, bot rơi về gửi
// thẳng inbox nên mất banner "phản hồi bình luận"; khách bình luận LẦN ĐẦU (chưa có
// hội thoại inbox) thì không nhận được gì.
// Nơi gọi ĐÃ biết page thật (conversation.page_id) — chỉ cần truyền vào.
function _pc(conversationId, _pageHint) {
  let pid = String(_pageHint || "").trim() || reg.pageIdFromConv(conversationId) || PAGE_ID;
...
function endpoint(conversationId, _pageHint) {
  const { pid, tok } = _pc(conversationId, _pageHint);
  return (
...
// [FIX Hà Giang 2026-08-26] "MẶC ĐỊNH COI LÀ THÀNH CÔNG" — lỗi im lặng nguy hiểm.
// Pancake thỉnh thoảng trả về CHUỖI thay vì object, ví dụ "Server internal error".
// Chuỗi là JSON hợp lệ, và chuỗi KHÔNG có thuộc tính .success -> `r.success !== false`
// ra true -> bot ghi nhận đã gửi, đặt cờ đã-trả-lời, KHÔNG BAO GIỜ thử lại.
// Đo trên page thật: câu trả lời công khai dưới bình luận thất bại mà log in "OK?: true".
// Nay: phản hồi không phải OBJECT thì coi là HỎNG.
function _thanhCong(r) {
  if (!r || typeof r !== "object" || Array.isArray(r)) return false;
  return r.success !== false;
}

// ===== KÝ TÊN MÁY GỬI — CHỈ DÙNG KHI CHẠY THỬ ===============================
// Một page đang có HAI bot: bản thử ở máy này và một bản khác ở máy khác. Nhìn
// từ Pancake KHÔNG phân biệt được ai gửi — cả hai đều hiện admin_name = "Public
// API" vì cùng gửi bằng page token (đo 27/08/2026 trên inbox thật). Pancake
// không cho đổi nhãn đó, và public_api cũng không có tham số tên người gửi.
// Nên chỗ duy nhất còn lại để ký là CUỐI CÂU TRẢ LỜI.
//
// KHÁCH NHÌN THẤY CHỮ KÝ NÀY. Vì vậy: TẮT MẶC ĐỊNH. Chỉ khai KY_TEN trong
// .env.staging (máy chạy thử). Bản thật chỉ đọc .env — không khai thì không ký,
// nên lúc vào vận hành thật không phải nhớ gỡ mã ở đây.
const KY_TEN = String(process.env.KY_TEN || "").trim();
const KY_DAU = String(process.env.KY_DAU || "—").trim();   // dấu đứng trước tên
if (KY_TEN) {
  try { console.log(`[ký tên] MỌI tin bot gửi sẽ kết thúc bằng "${KY_DAU} ${KY_TEN}" (khách ĐỌC ĐƯỢC) — bỏ KY_TEN khỏi .env là tắt.`); } catch (_) {}
}
// Thêm chữ ký vào cuối câu. Không khai KY_TEN -> trả lại y nguyên.
function kyTen(text) {
  const t = String(text == null ? "" : text).trim();
  if (!KY_TEN || !t) return t;
  const chuKy = `${KY_DAU} ${KY_TEN}`.trim();
  if (t.endsWith(chuKy)) return t;          // đã ký rồi -> đừng ký hai lần
  return `${t}
${chuKy}`;
}
...
async function sendInboxMessage(conversationId, text) {
  ...
  const msg = kyTen(text);
  const res = await fetch(endpoint(conversationId), {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ action: "reply_inbox", message: msg })
  });
  const _out = await res.json();
  if (_out && _out.success !== false) turnLog.sent("inbox", msg);
  return _out;
}

// TRẢ LỜI COMMENT CÔNG KHAI. action: "reply_comment", cần message_id = id comment của khách.
async function replyComment(conversationId, text, commentId, pageId) {
  if (!conversationId) throw new Error("Thiếu conversationId");
  if (!text || !String(text).trim()) return { success: false, reason: "EMPTY_MESSAGE" };
  const msg = kyTen(text);
  const body = { action: "reply_comment", message: msg };
  if (commentId) body.message_id = commentId;   // Pancake yêu cầu 'message_id'
  const res = await fetch(endpoint(conversationId, pageId), {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(body)
  });
  const _out = await res.json().catch(() => ({ success: false, reason: "BAD_JSON" }));
  if (_thanhCong(_out)) { turnLog.sent("comment", msg); return _out; }
  console.log("replyComment HỎNG | conv=" + conversationId + " | phản hồi:", JSON.stringify(_out));
  return { success: false, reason: "API_TU_CHOI", phanHoi: _out };
}
...
async function sendInboxMessageWithImages(conversationId, text, contentIds, imageUrls) {
  ...
  if (text && String(text).trim()) body.message = kyTen(text);
...
async function sendPrivateReply(conversationId, text, commentId, postId, pageId) {
  if (!conversationId || !commentId) return { success: false, reason: "MISSING" };
  if (!text || !String(text).trim()) return { success: false, reason: "EMPTY_MESSAGE" };
  const body = { action: "private_replies", message: kyTen(text), message_id: commentId };
  ...
  const _out = await res.json().catch(() => ({ success: false, reason: "BAD_JSON" }));
  return _thanhCong(_out) ? _out : { success: false, reason: "API_TU_CHOI", phanHoi: _out };
}
...
let _ghiChuApiSong = true;   // false khi đã biết Pancake không có endpoint ghi chú
async function addConversationNote(conversationId, text) {
  ...
  let last = null;
  // ĐO 26/08/2026: cả 5 đường ghi chú của Pancake đều trả 404 "Page not found"
  // -> public API KHÔNG có ghi chú. Thử một lần rồi thôi, đừng bắn 3 request
  // hụt cho MỖI ghi chú (bot có 6 chỗ gọi, mỗi lượt vài lần).
  if (_ghiChuApiSong) {
    ...
    _ghiChuApiSong = false;
    console.log("NOTE: API ghi chú Pancake KHÔNG dùng được (" + JSON.stringify(last) +
                ") -> từ giờ ghi thẳng vào sổ ngoài, xem bằng: node ghi_chu_ngoai.js");
  }
  // KHÔNG được im lặng nuốt: ghi chú là việc cần NGƯỜI làm (thêm dòng Sheet,
  // đối chiếu giá, lên đơn đủ mẫu). Mất nó thì khách ngồi chờ mà không ai biết.
  try { require("../tien_ich/ghi_chu_ngoai").ghi({ conversationId, text: msg }); } catch (_) {}
  return { success: true, qua_so_ngoai: true };
}

module.exports = { _thanhCong, kyTen, sendInboxMessage, replyComment, sendPrivateReply, sendInboxImage, sendInboxImages, sendInboxContentIds, sendInboxImageUrl, sendInboxImageUrls, sendInboxMessageWithImages, addTag, removeTag, tagChoXuLy, markUnread, tagChoXuLyVaUnread, tagXuLyAnh, tagXuLyAnhVaUnread, untagXuLyAnh, tagDonUuTien, tagDonUuTienVaUnread, tagGuiDonGap, tagAiChot, addConversationNote, delay };
