...
async function fetchPancakePost(storyFbid, pageId) {
  ...
  // NHỚ CẢ LẦN TRƯỢT. Trước đây chỉ cache khi có kết quả -> bài CŨ (ngoài 120 ngày,
  // Pancake trả null) bị đọc lại ở MỌI lượt, mỗi lượt 2 request + độ trễ, mà lần nào
  // cũng trượt. Đo trên page thật 25/08/2026: 7 lượt liên tiếp = 7 lần đọc, trượt cả 7.
  // Bài cũ thì mai cũng vẫn cũ — nhớ 30 phút rồi hẵng thử lại.
  // NGOẠI LỆ: 429 (bị bóp nhịp) là lỗi TẠM THỜI, KHÔNG cache — để lượt sau thử lại ngay.
  if (result) _postCache.set(sfb, { at: Date.now(), result });
  else if (!_adFetchRL) _postCache.set(sfb, { at: Date.now(), result: null });
  return result;
}
