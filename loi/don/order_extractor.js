...
const { colorMatches } = require("../san_pham/color_utils");
...
const { getPrice } = require("../san_pham/product_reply_rules");
...
let _store = null;
try { require("node:sqlite"); _store = require("../bo_nho/conversation_store"); } catch (_) {}
...
function buildOrderPlan(conversationId, conv) {
  ...
  // Mặc định: CHỈ xử lý tin chốt MỚI NHẤT = ĐƠN TỔNG hiện tại. Bot tư vấn re-confirm CẢ ĐƠN mỗi khi khách
  // thêm mẫu, nên tin chốt mới nhất đã gồm ĐỦ mọi mẫu -> tránh lên trùng mẫu cũ.
  //
  // [27/08/2026] "1 hội thoại = 1 đơn = 1 lần ship" KHÔNG còn đúng khi khách xin giao mỗi món một nơi
  // ("1c trắng ship về địa chỉ cũ, 1c đen ship về 67 Nguyễn Xiển"). Bot tư vấn nay phát MỖI NƠI GIAO một
  // tin chốt, đánh nhãn "(đơn i/N)" ngay dòng đầu. Thấy nhãn đó thì lấy TRỌN cụm N tin cuối.
  //
  // Vì sao dựa vào NHÃN chứ không dựa vào "địa chỉ khác nhau": khách đổi địa chỉ giữa chừng cũng sinh ra
  // hai tin chốt khác địa chỉ, mà đó là MỘT đơn đổi chỗ giao, không phải hai đơn. Nhãn do bot tự đánh nên
  // phân biệt được hai ca đó; đoán theo địa chỉ thì không.
  const _layMotCai = () => cods.length
  ...
  const _RE_NHAN = /\(\s*đơn\s*(\d+)\s*\/\s*(\d+)\s*\)/i;
  const _nhanCuoi = cods.length ? String(cods[cods.length - 1].text || "").match(_RE_NHAN) : null;
  const _soDon = _nhanCuoi ? Number(_nhanCuoi[2]) : 0;
  let nhieuNoiGiao = false;
  let codEntries;
  if (_soDon >= 2 && cods.length >= _soDon) {
    const cum = cods.slice(-_soDon);
    // Cụm phải đủ và đúng thứ tự 1..N, nếu không thì thà quay về luật cũ còn hơn lên đơn thiếu/thừa.
    const duCum = cum.every((c, k) => {
      const m = String(c.text || "").match(_RE_NHAN);
      return m && Number(m[2]) === _soDon && Number(m[1]) === k + 1;
    });
    if (duCum) {
      nhieuNoiGiao = true;
      codEntries = cum.map(c => ({ text: c.text, cod: c.cod, msg: messages[c.idx] }));
    }
  }
  if (!codEntries) codEntries = _layMotCai();
  ...
  for (const ce of codEntries) {
    ...
    // Cụm nhiều nơi giao: hai đơn CÙNG món khác nhà vẫn là hai đơn. Không kèm nơi giao vào chữ ký thì
    // đơn thứ hai bị coi là trùng rồi bỏ qua -> khách chỉ nhận được một nửa.
    if (nhieuNoiGiao && order.address) order.sig += `|@${fold(order.address)}`;
