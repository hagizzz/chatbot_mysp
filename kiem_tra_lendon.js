...
(async () => {
  ...
  try {
    const { getShops } = require("./pos_client");
  ...
  try {
    const { getConversations } = require("./pancake_reader");
    const { hasTag } = require("./conversation_tags");
  ...
  try {
    const { ensureProducts } = require("./pos_client");
